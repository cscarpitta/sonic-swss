...
#define SRV6_LOCALSID_MAX (__LOCALSID_MAX - 1)
...
enum {
	...
	__SRV6_LOCALSID_FORMAT_MAX,
};
#define SRV6_LOCALSID_FORMAT_MAX (__SRV6_LOCALSID_FORMAT_MAX - 1)
 ...
 * Return:      void.
 */
void RouteSync::parseEncapSrv6LocalSidFormat(struct rtattr *tb, string &block_len, string &node_len, string &func_len, string &arg_len)
{
    ...
    if (tb_localsid_format[SRV6_LOCALSID_FORMAT_BLOCK_LEN]) {
        block_len_buf = *(uint8_t *)RTA_DATA(tb_localsid_format[SRV6_LOCALSID_FORMAT_BLOCK_LEN]);
    ...
    if (tb_localsid_format[SRV6_LOCALSID_FORMAT_NODE_LEN]) {
        node_len_buf = *(uint8_t *)RTA_DATA(tb_localsid_format[SRV6_LOCALSID_FORMAT_NODE_LEN]);
    ...
    if (tb_localsid_format[SRV6_LOCALSID_FORMAT_FUNC_LEN]) {
        func_len_buf = *(uint8_t *)RTA_DATA(tb_localsid_format[SRV6_LOCALSID_FORMAT_FUNC_LEN]);
    ...
    if (tb_localsid_format[SRV6_LOCALSID_FORMAT_ARG_LEN]) {
        arg_len_buf = *(uint8_t *)RTA_DATA(tb_localsid_format[SRV6_LOCALSID_FORMAT_ARG_LEN]);
...
void RouteSync::parseEncapSrv6LocalSid(struct rtattr *tb, string &block_len, string &node_len, string &func_len, string &arg_len, string &action, string &vrf, string &adj)
{
    ...
    if (tb_encap[SRV6_LOCALSID_FORMAT])
        parseEncapSrv6LocalSidFormat(tb_encap[SRV6_LOCALSID_FORMAT], block_len, node_len, func_len, arg_len);
...
void RouteSync::onSrv6RouteMsg(struct nlmsghdr *h, int len)
{
    struct rtmsg *rtm;
    struct rtattr *tb[RTA_MAX + 1];
    void *dest = NULL;
    ...
    if (vrf_index)
    {
        ...
        {
            SWSS_LOG_ERROR("Invalid VRF name %s (ifindex %u)", routeTableKey, vrf_index);
            return;
        }
     ...
     * accompanied by any nexthops, therefore we expect all RTM_DELROUTE messages
     * to be processed by onMsg() using the rtnl api.
     */
    if (nlmsg_type == RTM_DELROUTE)
    {
        SWSS_LOG_ERROR("RTM_DELROUTE message-type not expected (%s)", destipprefix);
        return;
    }
    else if (nlmsg_type != RTM_NEWROUTE)
    {
        SWSS_LOG_ERROR("Unknown message-type: %d for %s", nlmsg_type, destipprefix);
        return;
    }
...
void RouteSync::onSrv6LocalSidRouteMsg(struct nlmsghdr *h, int len)
{
    ...
    SWSS_LOG_DEBUG("Rx MsgType:%d Family:%d DstAddress:%s/%d", h->nlmsg_type, rtm->rtm_family,
                    dstaddr_str, dst_len);
    ...
    if (nlmsg_type == RTM_DELROUTE)
    {
        SWSS_LOG_ERROR("RTM_DELROUTE message-type not expected (%s)", dstaddr_str);
        return;
    }
    else if (nlmsg_type != RTM_NEWROUTE)
    {
        SWSS_LOG_ERROR("Unknown message-type: %d for %s", nlmsg_type, dstaddr_str);
        return;
    }
    ...
    switch (rtm->rtm_type)
    {
        ...
        case RTN_PROHIBIT:
        {
            SWSS_LOG_ERROR("RTN_BLACKHOLE route not expected (%s)", dstaddr_str);
            return;
        }
        case RTN_UNICAST:
            break;
        ...
        case RTN_BROADCAST:
        case RTN_LOCAL:
            SWSS_LOG_NOTICE("BUM routes aren't supported yet (%s)", dstaddr_str);
            return;
    ...
    snprintf(my_sid_table_key, sizeof(my_sid_table_key), "%s",
            dstaddr_str);
