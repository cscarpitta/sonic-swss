...
SWSS_LOG_ERROR("parseEncapSrv6SteerRoute 1");
    parseRtAttrNested(tb_encap, 256, tb);
SWSS_LOG_ERROR("parseEncapSrv6SteerRoute 2");

    if (tb_encap[ROUTE_ENCAP_SRV6_VPN_SID])
    {
SWSS_LOG_ERROR("parseEncapSrv6SteerRoute 3");
    ...
    {
SWSS_LOG_ERROR("parseEncapSrv6SteerRoute 4");
        src_addr +=
...
{
    uint16_t encap = 0;
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 1");
    if (h->nlmsg_type == RTM_NEWROUTE)
    {
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 2");
        if (!tb[RTA_MULTIPATH])
        {
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 3");
            if (tb[RTA_ENCAP_TYPE])
            {
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 4");
                encap = *(uint16_t *)RTA_DATA(tb[RTA_ENCAP_TYPE]);
            }

SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 5");
            if (tb[RTA_ENCAP] && tb[RTA_ENCAP_TYPE] &&
                *(uint16_t *)RTA_DATA(tb[RTA_ENCAP_TYPE]) ==
                    NH_ENCAP_SRV6_ROUTE)
            {
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 6");
                parseEncapSrv6SteerRoute(tb[RTA_ENCAP], vpn_sid, src_addr);
            }
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 7");
            SWSS_LOG_DEBUG("Rx MsgType:%d encap:%d vpn_sid:%s src_addr:%s",
                           h->nlmsg_type, encap, vpn_sid.c_str(),
                           src_addr.c_str());

SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 8");
            if (vpn_sid.empty())
            {
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 9");
                return false;
            }
        }
        else
        {
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 10");
            /* This is a multipath route */
            SWSS_LOG_NOTICE("Multipath SRv6 routes aren't supported");
            return false;
        }
    }
SWSS_LOG_ERROR("getSrv6SteerRouteNextHop 11");
    return true;
}
...
SWSS_LOG_ERROR("onMsgRaw 4");

    switch (getEncapType(h))
    {
        case NH_ENCAP_SRV6_ROUTE:
SWSS_LOG_ERROR("onMsgRaw 5");
            onSrv6SteerRouteMsg(h, len);
SWSS_LOG_ERROR("onMsgRaw 6");
            break;
        default:
SWSS_LOG_ERROR("onMsgRaw 7");
            /*
