...
namespace ut_fpmsyncd
{
    ...
    TEST_F(FpmSyncdSRv6LocalSIDsTest, RecevingRouteWithSRv6LocalSIDEndDT4)
    {
        ...
        ASSERT_EQ(m_srv6LocalSidTable->hget("fc00:0:1:1::", "block_len", block_len), true);
        ASSERT_EQ(block_len, "32");

        std::string node_len;
        ASSERT_EQ(m_srv6LocalSidTable->hget("fc00:0:1:1::", "node_len", node_len), true);
        ASSERT_EQ(node_len, "16");

        std::string func_len;
        ASSERT_EQ(m_srv6LocalSidTable->hget("fc00:0:1:1::", "func_len", func_len), true);
        ASSERT_EQ(func_len, "16");

        std::string arg_len;
        ASSERT_EQ(m_srv6LocalSidTable->hget("fc00:0:1:1::", "arg_len", arg_len), true);
        ASSERT_EQ(arg_len, "0");

        std::string action;
        ASSERT_EQ(m_srv6LocalSidTable->hget("fc00:0:1:1::", "action", action), true);
        ASSERT_EQ(action, "end.dt4");

        std::string vrf;
        ASSERT_EQ(m_srv6LocalSidTable->hget("fc00:0:1:1::", "vrf", vrf), true);
        ASSERT_EQ(vrf, "Vrf10");
